...
void Preprocess::avia_handler(const livox_ros_driver::CustomMsg::ConstPtr &msg)
{
  ...
  ROS_WARN_ONCE("range from %f to  %f m.", blind, max_blind);
  if (feature_enabled)
  {
    ROS_WARN_ONCE(" feature_enabled");
    for (uint i = 1; i < plsize; i++)
    {
      if ((msg->points[i].line < N_SCANS) && ((msg->points[i].tag & 0x30) == 0x10 || (msg->points[i].tag & 0x30) == 0x00))
      {
        ...
        if (pl_full[i].x < blind || pl_full[i].x > max_blind)
        {
          continue;
        }
        ...
        // if (msg->points[i].reflectivity > 50)
        // {
        //   continue;
        // }
...
void Preprocess::RobosenseHandler(const sensor_msgs::PointCloud2::ConstPtr &msg)
{
  ...
  // ROS_ERROR("pl_orig->size()is %d",  pl_orig.size());
  // ROS_ERROR_ONCE("RobosenseHandler" );
  // ROS_ERROR("  %f  %f   %d",  max_blind, blind , point_filter_num);
  ...
  if ( 1 )
  {
    ...
    for (int i = 0; i < plsize; i++)
    {
      ...
      // if ( added_pt.x < -200 || added_pt.x > 200 ||
      //      added_pt.y < -15 || added_pt.y > 15 ||
      //      added_pt.y < -3 || added_pt.y > 5 )
      // {
      //     continue;
      // }

      // 机械雷达360 不适应这个样的写法 固态的可以
      // if (added_pt.x < blind || added_pt.y < blind || added_pt.z < blind) // blind 认为是 雷达的有效探测范围
      // {
      //   continue;
      // }
      // if (added_pt.x > max_blind || added_pt.y > max_blind || added_pt.z > ) // max_blind 认为是 雷达的有效探测范围
      // {
      //   continue;
      // }
